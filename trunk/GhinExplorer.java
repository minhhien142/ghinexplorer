/**************************************************
 *  *   Copyright (c) 2006 Javageo.com. All Rights Reserved.
 * 
...
public class GhinExplorer extends JFrame implements ActionListener
{
    ...
    public JMenuItem openwithPop,opennewPop, copyPop,pastePop, renamePop,newdirPop,newfilePop,deletePop,openmspaintPop,openeditorPop,
            openbrowserPop,opendocumentPop,openexcelPop,openjavaPop,propertiesPop;
 	...
 	public GhinExplorer()
	{
        ...
        myTree.addTreeSelectionListener(new TreeSelectionListener()
	   {
		...
		{ 
	       ...
	       int indexHome = selRow-1;
	       	...
	       	JOptionPane.showMessageDialog(null,"Error select file directory "+ex,
         ...
         PropertyChangeListener mylisterner = new PropertyChangeListener()
		{
		 ...
		 pastePop.setVisible(false); //hide first
		...
		//if you like to make JTree has popup do this..
		// myTree.setComponentPopupMenu(popup);
       ...
       ActionListener actionlist = new ActionListener() 
		{ 
			public void actionPerformed(ActionEvent ev)
			{
		       ...
		       {			 
		        ...
		        //open file with your program ..(keep world green!)
		       ...
		       // System.out.println("peace and green "+ dir);
		         }
		         
		         if(stringChoose=="CancelSelection")
		         {	// whatever you like to call in cancel button lah...
		...
		findfilename.setVisible(false); // hide first
		...
		protected JMenuBar createMenuBar()
	{
		...
		{ 
			public void actionPerformed(ActionEvent e)
			{
		        ...
		        JLabel label2 = new JLabel("<html><li>Ghin Explorer 1.0™ "
		        + "</li><li><p>Ver# 1.0 </li>"
		        + "<li><p>Develop by: Goen-Ghin</li><li><p>JavaGeo Technology System</li><li>"
		        + "<p>Copyright<font size=\"2\">©</font> June 2008 @Pekanbaru Riau Indonesia</li></html>");
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (ae.getSource() == toolbar.button[0]) 
		{
 		   ...
 		   if(selRow > 0) myTree.setSelectionRow(selRow) ;
           else selRow=0;
		...
		{
			int selRow = myTree.getLeadSelectionRow()+1 ;
 		   if(selRow > 0 || selRow <= myTree.getRowCount() ) myTree.setSelectionRow(selRow) ;
 		   else selRow=0;
...
public static void copy(String fromFileName, String toFileName)
    ...
    try {
      ...
      while ((bytesRead = from.read(buffer)) != -1)
        to.write(buffer, 0, bytesRead); // write file
    } finally {
      if (from != null)
        try {
          from.close();
...
class menuListener implements ActionListener {  
		public void actionPerformed(ActionEvent e) {
			//run file with any program that you like to run
				...
				if(JOptionPane.showConfirmDialog(null,"Delete This File ?","Delete File", JOptionPane.YES_NO_OPTION )
					== JOptionPane.YES_OPTION)
                    myFileExplorer.getSelectedFile().delete();
				...
				if (e.getActionCommand() == "Paste") 
				{
					...
					//System.out.println("dir to copy"+mycopyFileName.toString());
					//System.out.println("copy "+mycopyPath+File.separatorChar+mycopyFile+" paste to "+myFileExplorer.getCurrentDirectory().toString()+File.separatorChar+mycopyFile);
			        // need to figure out to copy directories and it sub directories...
			           if(mycopyFileName.isDirectory()) copyDirectory(mycopyFileName,myFileExplorer.getCurrentDirectory());
					   else
					   copy(mycopyPath+File.separatorChar+mycopyFile,myFileExplorer.getCurrentDirectory().toString()+File.separatorChar+mycopyFile);
						
					  myFileExplorer.rescanCurrentDirectory() ;
						
					      } catch(IOException e){JOptionPane.showMessageDialog(null, 
				"Error to copy file/directory "+e,
				"Warning", JOptionPane.WARNING_MESSAGE);}		 
    				}
					};
					runner9.start();
				...
				if (e.getActionCommand() == "New Directory") 
				{
					...
					//create empty directory
                        ...
                        else if(inputValue!=null) dirstatus = new File(myFileExplorer.getCurrentDirectory().toString()+File.separatorChar+inputValue).mkdir();                        
 						myFileExplorer.rescanCurrentDirectory() ;
     				}
					};
					runner10.start();
...
class FindFileName extends JPanel
{
	...
	FindFileName (final JFileChooser chooser)
	{
		...
		{
			JOptionPane.showMessageDialog(null,"Error run program file "+e,
				"Warning", JOptionPane.WARNING_MESSAGE);
		  }
                 }
              };
		...
		{ 
			...
			{
			   // stop find file 
			   // --- if you find your jlist empty click find again 
			   // --- there is delay a half second from insert data to put in jlist
			   // --- need to improve the renderer
	...
	//run search file
	protected void runFileFind (File base)
					throws InterruptedException
	{
		...
		for (int i=0; i<files.length; i++)
		{
            ...
            //run find again into sub directories 
		...
		class FileFindResults extends JPanel
	{
				...
				public void mouseClicked (MouseEvent e)
				{
					if (e.getClickCount() == 2)
					{
						try
						{
							...
							//System.out.println("fileku"+ fileinih.toString());
							//show the file name directory in jfilechooser after you double click...
							getChooser();
						}
						catch (Throwable err)
						{ //ignore
		...
		public File getFileSelect()	{  return fileinih; }
		...
		*/
		...
		{ 
		//icon for file in list (you can look java documentation of this class)
			...
			public Icon getIcon(File f)
	{
		...
		if (name.endsWith(".doc"))
			...
			return JAVA_ICON;
		else if (name.endsWith(".html")||name.endsWith(".htm")||name.endsWith(".php"))
			return WEB_ICON;
		else if (name.endsWith(".zip")||name.endsWith(".gz"))
			return ZIP_ICON;
		else if (name.endsWith(".au")||name.endsWith(".mpeg"))
			return MEDIA_ICON;	
		else if (name.endsWith(".exe")||name.endsWith(".com")||name.endsWith(".sh")||name.endsWith(".bat"))
			return RUN_ICON;
		else if (name.endsWith(".class"))
			return CLASS_ICON;	
		else if(f.isFile())  return DOKU_ICON;
...
class DirView extends javax.swing.filechooser.FileView
{
	...
	public Icon getIcon(File f)
	{
		//you can add more file's icon here
		String name = f.getName().toLowerCase();
		if (name.endsWith(".doc"))
			return DOKU_ICON;
		else if (name.endsWith(".txt"))
...
class FileNode
{
	...
	public boolean expand(DefaultMutableTreeNode parent)
	{
		...
		{
			FileNode nd = (FileNode)v.elementAt(i);
			IconData idata = new IconData(GhinExplorer.ICON_FOLDER, 
				GhinExplorer.ICON_EXPANDEDFOLDER, nd);
